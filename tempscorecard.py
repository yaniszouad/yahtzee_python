...
class Scorecard:
    ...
    def create_scorecard(self, game_id, user_id, turn_order):
        try:
            ...
            results = cursor.execute("SELECT * FROM scorecards WHERE game_id = ? AND user_id = ?;", (game_id, user_id))
            if results.fetchall():
    ...
    def update_scorecard(self, id, score_info):
        ...
        try:
            ...
            if not updated_scorecard:
                return {"result": "error", "message": "Scorecard does not exist"}
            return {"result": "success", "message": self.dict_transformer(updated_scorecard)}
    ...
    def remove_scorecard(self, id):
        ...
        if id:
            ...
            if not game:
                return {"result":"error", "message": "scorecard doesnt exist"}
        try:
            cursor.execute(f"DELETE FROM {self.table_name} WHERE id = ?", (id,))
            db_connection.commit()
            return {"result": "success", "message": "Scorecard removed successfully"}
        except sqlite3.Error as error:
            return {"result": "error", "message": str(error)}
        finally:
            db_connection.close()
    ...
    def tally_score(self, score_info):
        total_score = 0
        for scores in score_info.items():
